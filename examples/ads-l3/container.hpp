...
class t234 : public tuim::IGraphExportable<Key>
{
private:
    struct Node;
...
public:
    ...
    public: // operators
        ...
        { return node_ == other.node_ && key_num_ == other.key_num_; }
...
private:
    ...
    public:
        ...
        keys() const override
        {
            std::vector<const_pointer> pointers(size());
            for (size_t n = 0; n < size(); ++n)
                pointers[n] = &*keys_[n];
            return pointers;
        }

        tuim::IGraphNodePtrs<value_type>
        nodes() const override
        {
            tuim::IGraphNodePtrs<value_type> pointers(size() + 1);
